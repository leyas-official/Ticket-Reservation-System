...
<div class="flex-1 p-10 mt-16 ml-56">
    <div class="flex justify-between mb-4">
        <h2 class="text-2xl font-bold">Event Types</h2>
        <a href="{{ route('admin.eventTypes.create') }}" class="bg-blue-800 hover:bg-blue-900 text-white font-bold py-2 px-4 rounded transition duration-200 font-semibold"> Add Event Type </a>
    </div>
    @if(session('success'))
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full bg-gray-100 border border-gray-200 rounded-md text-sm">
            ...
            <tr class="bg-gray-700 text-white uppercase text-sm leading-normal">
                ...
                <th class="py-3 px-6 text-left">Events Count</th>
            ...
            @foreach ($eventTypes as $eventType)
                <tr class="border-b border-gray-200 hover:bg-gray-300 transition duration-200 cursor-pointer">
                    <td class="py-3 px-6 truncate">{{ $eventType->id }}</td>
                    <td class="py-3 px-6 truncate ... max-w-40 hover:max-w-60 transition-all duration-300">{{ $eventType->name }}</td>
                    <td class="py-3 px-6">{{ $eventType->events_count }}</td>
                    <td class="py-3 px-6 flex flex-row gap-4">
                        <a href="{{ route('admin.eventTypes.edit' , $eventType->id) }}" class="bg-green-600 text-white py-1 px-3 rounded hover:bg-green-700 transition duration-200 font-semibold">Edit</a>
                        <button type="button" class="bg-red-500 text-white py-1 px-3 rounded hover:bg-red-600 transition duration-200 font-semibold" onclick="showConfirmationModal('{{ $eventType->id }}')">
                            Delete
                        </button>
                        <form id="delete-form-{{ $eventType->id }}" action="{{ route('admin.eventTypes.delete', $eventType->id) }}" method="POST" class="hidden">
                            @csrf
                            @method('delete')
                        </form>
                    </td>
