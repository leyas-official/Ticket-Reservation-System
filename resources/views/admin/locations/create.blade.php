...
<div class="max-w-xl mx-auto mt-16 bg-white rounded-xl shadow-md p-8">
    <h2 class="text-3xl font-bold text-center text-blue-800 mb-6">Add New Location</h2>
    <form action="{{ route('admin.locations.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" id="address" name="address" required
                   ...
                   placeholder="Enter Name Event" value="{{ old('address') }}">
            @error('address')
            <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="text" id="capacity" name="capacity" required
                   class="w-full border @error('capacity') border-red-500 border-gray-300 @enderror p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" value="{{ old('capacity') }}" placeholder="Capacity">
            @error('capacity')
            <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
